...
# 构建boardInfo.json文件的绝对路径
BOARD_INFO_PATH = os.path.join(CURRENT_DIR, 'boardInfo.json')

try:
    with open(BOARD_INFO_PATH, 'r', encoding='utf-8') as f:
        board_info = json.load(f)
    print(f"✅ 成功加载 boardInfo.json 文件，路径: {BOARD_INFO_PATH}")
    print(f"   包含的键: {list(board_info.keys())}")
except FileNotFoundError:
    print(f"错误: 请确保 boardInfo.json 文件在目录 {CURRENT_DIR} 中")
    board_info = {}
